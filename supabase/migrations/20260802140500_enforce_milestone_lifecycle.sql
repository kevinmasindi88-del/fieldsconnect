-- Enforce controlled mentorship milestone lifecycle.
...
begin
  ...
  if caller_id not in (
    ...
    raise exception
      'Only mentorship participants may manage milestones';
  end if;
  ...
  if tg_op = 'INSERT' then
    if caller_id <> new.created_by then
      raise exception
        'The milestone creator must be the signed-in participant';
    end if;
...
mentorship_milestones_lifecycle
